...
crate::error_stack::hook::context::impl_hook_context! {
...
fn into_boxed_hook<T: Send + Sync + 'static>(
    ...
    Box::new(move |frame: &Frame, context: &mut HookContext<Frame>| {
        ...
        #[cfg(not(nightly))]
        matches!(frame.kind(), error_stack::FrameKind::Attachment(_))
            .then_some(frame)
...
mod default {
    #[cfg(any(all(not(nightly), feature = "std"), feature = "spantrace"))]
    use alloc::format;
    #[cfg_attr(feature = "std", allow(unused_imports))]
